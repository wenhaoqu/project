...
int SnakeMap::getGameRecord()
{
    ...
    char rec[128] = {0};
    fgets(rec, MAX_LINE, fp);

    fclose(fp);
    return atoi(rec);
}
void SnakeMap::updateGameRecord()
{
...
{
    if (snake->length > max_score) max_score = snake->length;
    cout << "Score:" << snake->length << endl << "Max Score:" << max_score << endl;
}